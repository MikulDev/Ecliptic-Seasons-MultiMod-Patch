...
public class FetzisHandler {
    ...
    public void onTagsUpdatedEvent(TagsUpdatedEvent tagsUpdatedEvent) {
        ...
        try {
            ...
            String packageName = "io.github.leonard1504.blocks";

            Set<Class<?>> classes = getClassType(packageName, List.of("HandRail", "Panel", "VerticalSlab"));
            Set<Class<?>> classesSolid = getClassType(packageName, List.of("RoofTop", "TallRoofTop", "ZenBlock", "SlabRoof"));
            classesSolid.add(StairBlock.class);

            Class<?> initClass = Class.forName("io.github.leonard1504.init.blockInit");
            ...
            while (iterator.hasNext()) {
                ...
                if (!checkif(classes, block, block.builtInRegistryHolder(), info)) {
